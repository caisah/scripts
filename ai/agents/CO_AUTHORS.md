Add a `Co-authored-by` trailer for each distinct tool or model involved in authoring the commit. If the CLI host and the underlying model have separate trailers, include BOTH. Use a single contiguous trailer block for it.
